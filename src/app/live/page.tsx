...
function LivePageClient() {
  ...
  const getClientTestUrl = (rawUrl: string, source?: LiveSource | null) => {
    ...
    if (isM3u || !isProgressive) {
      return `/api/proxy/m3u8?url=${encodeURIComponent(rawUrl)}&movie-source=${encodeURIComponent(source?.key || '')}${proxyMode === 'm3u8-only' ? '&allowCORS=true' : ''}`;
    }
            ...
            if ((context as any).type === 'manifest') {
              if (proxyMode === 'full') {
                ...
                try {
                  ...
                  url.searchParams.set('movie-source', currentSourceRef.current?.key || '');
  ...
  useEffect(() => {
    const preload = async () => {
      ...
      if (proxyMode === 'direct') {
        ...
        try {
          const precheckUrl = `/api/live/precheck?url=${encodeURIComponent(videoUrl)}&movie-source=${currentSourceRef.current?.key || ''}`;
      ...
      if (type === 'm3u8') {
        if (proxyMode === 'direct') {
          ...
          targetUrl = `/api/proxy/m3u8?url=${encodeURIComponent(videoUrl)}&movie-source=${currentSourceRef.current?.key || ''}`;
        }
      }
