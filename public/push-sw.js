...
self.addEventListener('push', (event) => {
  ...
  try {
    ...
    payload = { title: 'MoviePlus', body: event.data.text() };
  }

  const title = payload.title || 'MoviePlus';
  const options = {
 ...
 * Route: /__MovieTV_idb_video__/<encoded-cache-key>/playlist.m3u8
 *        /__MovieTV_idb_video__/<encoded-cache-key>/segment_00000.ts
 *        /__MovieTV_idb_video__/<encoded-cache-key>/key.key
 */
const IDB_VIDEO_DB_NAME = 'MoviePlusVideoCache';
const IDB_VIDEO_DB_VERSION = 1;
const IDB_VIDEO_ROUTE_PREFIX = '/__MovieTV_idb_video__';
